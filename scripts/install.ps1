...
Invoke-Checked $Python -c "import platform,struct,sys; ok=(sys.version_info[:2]==(3,12) and struct.calcsize('P')*8==64 and platform.machine().upper()=='AMD64'); print('Python:',sys.version.split()[0],platform.machine(),str(struct.calcsize('P')*8)+'-bit'); raise SystemExit(0 if ok else 3)"
...
Invoke-Checked $Python -m pip install --only-binary=:all: --no-deps --require-hashes --force-reinstall -r $TikTokLock

Invoke-Checked $Python -c "from importlib import metadata; exp={'yt-dlp':'2026.8.19','curl_cffi':'0.16.2','cffi':'2.1.1','pycparser':'3.0','certifi':'2026.7.22'}; got={k:metadata.version(k) for k in exp}; print('Reviewed dependency read-back:',got); raise SystemExit(0 if got==exp else 4)"
Invoke-Checked $Python -c "from playwright.sync_api import sync_playwright; import faster_whisper, imageio_ffmpeg; print('OK: dependencies import successfully'); print('Bundled ffmpeg:', imageio_ffmpeg.get_ffmpeg_exe())"
