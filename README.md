...
The runtime expects the host control directory one level above the repository (for example `C:\ClaudeCode\control`). On a fresh installation, initialize the required settings file from the tracked non-secret template:
